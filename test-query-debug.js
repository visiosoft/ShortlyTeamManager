...
async function testQueryDebug() {
  console.log('🔍 Testing Query Debug...\n');
  ...
  // Get user profile to get teamId
  ...
  const expectedUrls = assignedUrls.urls.filter(url => 
    defaultUrlCreators.includes(url.createdByAdmin?.id)
  );

  console.log(`\n📊 Expected Result:`);
  console.log(`   - Should show: ${expectedUrls.length} URLs`);
  console.log(`   - Actually showing: ${assignedUrls.urls.length} URLs`);
  
  if (expectedUrls.length !== assignedUrls.urls.length) {
    console.log('❌ FAILED: Query is not filtering correctly!');
    console.log(`   - Expected: ${expectedUrls.length} URLs`);
    console.log(`   - Actual: ${assignedUrls.urls.length} URLs`);
    console.log(`   - Difference: ${assignedUrls.urls.length - expectedUrls.length} extra URLs`);
  } else {
    console.log('✅ SUCCESS: Query is filtering correctly!');
  }

  console.log('\n🎉 Query debug completed!');
}

testQueryDebug().catch(console.error); 