...
async function debugTeamUrls() {
  try {
    console.log('🔍 Debugging Team URLs and Template URLs...\n');
    ...
    defaultUrlsResponse.data.forEach((url, index) => {
      ...
      console.log(`     Team ID: ${url.teamId || 'NULL (Global)'}`);
    ...
    console.log('\n🔍 Analysis:');
    console.log(`- Admin Team ID: ${adminUser.teamId}`);
    console.log(`- New User Team ID: ${newUser.teamId}`);
    console.log(`- Template URLs are team-specific: ${defaultUrlsResponse.data.some(url => url.teamId)}`);
    console.log(`- Template URLs are global: ${defaultUrlsResponse.data.some(url => !url.teamId)}`);
